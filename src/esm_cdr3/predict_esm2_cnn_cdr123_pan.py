...
seq_names = ['peptide', 'A3', 'B3']
inp_names = ['pep','a3','b3']
max_seq_lens = [pep_max, a3_max, b3_max]
...
def make_tf_ds(df, hdf5_path):
    ...
    with h5py.File(hdf5_path, "r") as hdf5_file:    
      ...
      #  encoded_b2 = hdf5_file["B2"][:]
        ...
        print('Peptide embedding dimension: ', encoded_pep.shape)
        print('a3 embedding dimension: ', encoded_a3.shape)
   ...
   #     )
    ...
    # 检查其他数据的维度是否与模型要求一致
   # if encoded_a3.shape[-1] != 1280 or encoded_b3.shape[-1] != 1280:
   #     raise ValueError(
   #         "One of the CDR embeddings (A3, B3) has a dimension mismatch. Expected 1280."
   #     )


    targets = df.binder.values
    tf_ds = [np.float32(encoded_pep),
             #np.float32(encoded_a1), np.float32(encoded_a2), 
             np.float32(encoded_a3),
             #np.float32(encoded_b1), np.float32(encoded_b2), 
             np.float32(encoded_b3),
             targets]
...
for t in train_parts:
    ...
    #df = pd.read_csv( )
    try:
        tf_ds = make_tf_ds(data, args_dict['hdf5_path'])
        for i, dat in enumerate(tf_ds[:-1]):
    ...
    x_test = test_tensor[0:3]          # Features
    targets_test = test_tensor[3]      # Target values
