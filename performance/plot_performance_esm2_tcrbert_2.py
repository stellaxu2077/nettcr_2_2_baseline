...
data_esm2_cdr3 = pd.read_csv('m3a_auc01.tsv', sep='\t')
data_tcrbert_cdr3 = pd.read_csv('m3b_auc01.tsv', sep='\t')

# 数据集合和对应标签
#datasets = [data_esm2, data_tcrbert, data_esm2_tcrbert, data_esm2_cdr3, data_tcrbert_cdr3]
datasets = [
            data_tcrbert_cdr3,   
            data_tcrbert, 
            data_esm2_tcrbert
...
#labels = ['Baseline', 'ESM-2 (CDR123)', 'ESM-2 (CDR3)', 'ESM-2 (CDR12) + TCR-BERT (CDR3)', 'BLOSUM (CDR12) + TCR-BERT (CDR3)', 'TCR-BERT (CDR3)']
labels = ['TCR-BERT (CDR3)',
          'BLOSUM (CDR1/2) + TCR-BERT (CDR3)', 
          'ESM-2 (CDR1/2) + TCR-BERT (CDR3)'
          ]

# 按Peptide合并数据
merged_data = pd.concat([data_esm2[['Peptide', 'Positive_Count']],
...
# 保存并显示图表
plt.tight_layout()
plt.savefig('../plots/auc01_esm2_tcrbert_bar_chart2.png')
plt.show()
